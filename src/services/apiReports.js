...
export async function fetchPauses(filters) {
  ...
  let query = supabase
    .from('pauses')
    .select(
      'id, agent_id, pause_type_id, started_at, ended_at, duration_seconds, atraso, machine_ip, pause_types(code,label,limit_minutes,tolerance_start_minutes,tolerance_end_minutes), profiles!pauses_agent_id_fkey(full_name, team_id)'
    )
