-- Migration: only mark a pause late when the next full minute is reached
...
begin
  ...
  if v_limit_minutes is not null and v_limit_minutes > 0 then
    -- A 20-minute pause is allowed through 20:59; late starts at 21:00.
    v_is_late := v_duration >= ((v_limit_minutes + 1) * 60);
  end if;
...
begin
  ...
  where p.ended_at is null
    ...
    and extract(epoch from (v_now - p.started_at)) >= ((coalesce(ps.duration_minutes, pt.limit_minutes) + 1) * 60)
    and (
...
begin
  ...
  with active as (
    ...
    where p.ended_at is null
      and coalesce(ps.duration_minutes, pt.limit_minutes) is not null
      and coalesce(ps.duration_minutes, pt.limit_minutes) > 0
      and extract(epoch from (now() - p.started_at)) >= ((coalesce(ps.duration_minutes, pt.limit_minutes) + 1) * 60)
  )
